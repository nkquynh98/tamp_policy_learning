...
from policy_learning.model.ToyMotionNet import Toy_movetopick
...
#Init the tasknet
motionnet = Toy_movetopick(observation_dim=8)
motionnet.to(device)
    ...
    with torch.no_grad():
        for i, return_value in enumerate(test_loader):
            ...
            if i % CHECKPOINT_AFTER == 0:    # print every 5 mini-batches
                ...
                writer.add_scalar("movetopick/test_lost", running_loss / CHECKPOINT_AFTER, epoch*len(test_loader)+i)
                running_loss = 0.0         
